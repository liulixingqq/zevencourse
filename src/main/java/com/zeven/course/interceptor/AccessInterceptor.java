...
/**
 * Created by fangf on 2016/5/21. OAuth权限认证
 */
public class AccessInterceptor extends CrossDomainInterceptor{
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception{
		...
		if(!uri.startsWith(path + "/Access")){
			...
			if(auth == null){
				response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "Authentication Failed: Require Authorization");
				return false;
			}
			Token token = new Token(auth);
			if(token.getErr() == Token.ExpiredJwtError){
				response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "Authentication Failed: 认证过期");
				return false;
			}
			if(token.getErr() == Token.SignatureError){
				response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "Authentication Failed: 非法认证");
				return false;
			}
			...
			if(isAdmin || isTeacher || isStudent){
				response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "Authentication Failed: 无权访问");
				return false;
			}
		}
		return true;
