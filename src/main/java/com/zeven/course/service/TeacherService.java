...
public class TeacherService extends DaoSupportImpl<Teacher>{
	...
	public Teacher login(String tno, String password){
		List teachers = getSession().createQuery("FROM Teacher WHERE tno = (:tno)").setParameter("tno", tno).list();
		Teacher teacher = teachers.size() == 0 ? null : (Teacher)teachers.get(0);
		if(teacher != null) return teacher.getPassword().equals(MD5Encoder.encode(password)) ? teacher : null;
		else return null;
	}
	...
	public void deleteCascade(Integer[] ids){
		...
		session.createQuery("DELETE FROM TeacherCourse WHERE tid in (:ids)").setParameterList("ids", ids).executeUpdate();
		session.createQuery("DELETE FROM Teacher WHERE id IN (:ids)").setParameterList("ids", ids).executeUpdate();
		tx.commit();
