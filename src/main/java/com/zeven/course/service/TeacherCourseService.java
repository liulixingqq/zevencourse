...
public class TeacherCourseService extends DaoSupportImpl<TeacherCourse>{
	...
	public Message selectCourses(int userID, Integer[] ids){
		...
		if(session.createQuery("FROM TeacherCourse WHERE tid = (:tid) and cid in (:cid)").setParameter("tid", userID).setParameterList("cid", ids).list()
				.size() > 0){
	...
	public Message unselectCourses(int userID, Integer[] ids){
		Session session = getSession();
		Transaction tx = session.beginTransaction();
		int rows = session.createQuery("DELETE FROM TeacherCourse WHERE tid = (:tid) and cid in (:cid)").setParameter("tid", userID)
