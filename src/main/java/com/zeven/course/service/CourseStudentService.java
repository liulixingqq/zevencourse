...
public class CourseStudentService extends DaoSupportImpl<CourseStudent>{
	...
	public Message selectCourses(int userID, Integer[] ids){
		...
		if(session.createQuery("FROM CourseStudent WHERE sid = (:sid) and cid in (:cid)").setParameter("sid", userID).setParameterList("cid", ids).list()
				.size() > 0){
	...
	public Message unselectCourses(int userID, Integer[] ids){
		Session session = getSession();
		Transaction tx = session.beginTransaction();
		int rows = session.createQuery("DELETE FROM CourseStudent WHERE sid = (:sid) and cid in (:cid)").setParameter("sid", userID)
	...
	public List getGradeByCid(int id){
		return getSession().createQuery("FROM CourseStudent WHERE cid = (:cid)").setParameter("cid", id).list();
	}
	
	public Message recordGrade(int tid, int cid, int sid, double grade){
		Session session = getSession();
		if(session.createQuery("FROM TeacherCourse WHERE tid = (:tid) and cid = (:cid)").setParameter("tid", tid).setParameter("cid", cid).list().size() == 0)
			return new Message(-1, "您非此课程的授课教师，无法录入成绩");
		Transaction tx = session.beginTransaction();
		int row = session.createQuery("UPDATE CourseStudent SET grade = (:grade) WHERE cid = (:cid) and sid = (:sid)").setParameter("grade", grade)
				.setParameter("cid", cid).setParameter("sid", sid).executeUpdate();
		tx.commit();
	...
	public Map<String, List> queryGrade(int sid){
		...
		Session session = getSession();
		List grades = session.createQuery("FROM CourseStudent WHERE sid = (:sid) and grade != 0").setParameter("sid", sid).list();
		...
		else courses = session.createQuery("FROM Course WHERE id in (:cid)").setParameterList("cid", cids).list();
